	...
	return (
		<div className="relative">
			<div
				...
				style={{ backgroundImage: `url(${Shape.src})`, backgroundPosition: 'left 20%' }}
		/>
		...
		<div className="w-full lg:w-1/2">
			...
			<p className="text-center lg:text-left mb-3 text-gray-800">Easily monitor the status of your support requests in real-time. E
				nter your ticket number or EAC email to track your tickets.</p>
			<form onSubmit={handleSearch} className="flex justify-center lg:justify-start items-center mb-10">
			<div className="relative w-full max-w-md">
				...
				onFocus={(e) => { e.target.style.boxShadow = '0 0 0 3px #7E57C2'; }}
				...
				disabled={loading}
				>
				{loading ? <Loader2 className="animate-spin" width={20} height={20} /> : <SendHorizontal width={20} height={20} />}
				</button>
			</div>
			</form>
		</div>
			...
			{loading && (
				<div className="flex justify-center">
					<div className="flex items-center justify-center space-x-2">
						...
						<div className="h-2 w-2 bg-[#34BFA3] rounded-full animate-bounce" style={{ animationDelay: "0.2s" }}></div>
						<div className="h-2 w-2 bg-[#34BFA3] rounded-full animate-bounce" style={{ animationDelay: "0.4s" }}></div>
					</div>
				</div>
			)}
			...
			{searchEmail && !loading && (
				<div className="mb-6">
					<h2 className="text-xl font-semibold text-black">
						{tickets.length > 0 ? `Found ${tickets.length} ticket${tickets.length === 1 ? "" : "s"} for ${searchEmail}` : `No tickets found for ${searchEmail}`}
					</h2>
				</div>
			)}
			...
			<div className="grid grid-cols-1 gap-6">
				{tickets.map((ticket) => (
					<div key={ticket.ticket_id} className="bg-white border rounded-xl shadow-sm hover:shadow-md transition p-6">
						...
						<div className="mt-4">
							<p className="text-sm mb-1" style={{ color: "#34BFA3" }}>Category</p>
							<p className="text-gray-800">{ticket.category}</p>
						</div>

						<div className="mt-4">
							<p className="text-sm mb-1" style={{ color: "#34BFA3" }}>Description</p>
							<p className="text-gray-800 bg-gray-50 p-3 rounded-md">{ticket.description}</p>
						</div>

						<div className="mt-4">
							<p className="text-sm mb-1" style={{ color: "#34BFA3" }}>Priority Level</p>
				...
				{!loading && tickets.length === 0 && searchEmail && (
					<div className="text-center p-8 bg-gray-50 rounded-lg border border-gray-200">
						<Search className="mx-auto text-[#34BFA3] mb-2" size={32} />
						<p className="text-[#7E57C2] text-m">No support tickets found for this email.</p>
					</div>
				)}
			</div>
		</div>
